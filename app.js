...
export default {
    async fetch(req, env) {
      console.log(`async fetch ${req} : ${env}`);
      ...
      server.addEventListener("message", async (event) => {
        try {
          ...
          if (data.event === "media") {
            ...
            // Send to Gemini realtime
            const gemini = new WebSocket(
              `wss://generativelanguage.googleapis.com/v1beta/realtime?key=${env.GEMINI_API_KEY}`
            );
            ...
            gemini.addEventListener("message", async (m) => {
